...
function getTranslation(group, value) {
    ...
    try {
        ...
        console.warn("Käännöstiedostoa ei löytynyt, käytetään oletuksia.");
    }
    ...
    if (slice.length === 0 && displayedCount === 0) {
        ...
        sentinel.innerHTML = isFiltered 
            ? `<div style="padding: 20px;">
                <p>${t('msg.no_news_found_with_the_selected_filters')}</p>
                <button onclick="resetFilters()" style="background:var(--text-dark); color:white; border:none; padding:8px 15px; cursor:pointer; font-size:0.7rem; font-weight:700; text-transform:uppercase;">${t('ui.reset_filters')}</button>
               </div>`
            : t('msg._nothing_new_here_right_now_');
